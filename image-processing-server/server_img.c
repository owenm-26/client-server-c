...
int add_to_queue(struct request_meta to_add, struct queue * the_queue)
{
	...
	/* OPTION 2: Find where to place the request in the
	   ...
	   * position to the right. */
			...
			while (i != the_queue->rd_pos && timespec_cmp(&the_queue->requests[(i - 1 + the_queue->max_size) % the_queue->max_size].request.req_length, &to_add.request.req_length) > 0) {
				the_queue->requests[i] = the_queue->requests[(i - 1 + the_queue->max_size) % the_queue->max_size];
				i = (i - 1 + the_queue->max_size) % the_queue->max_size;
			}
			the_queue->requests[i] = to_add;
			the_queue->wr_pos = (the_queue->wr_pos + 1) % the_queue->max_size;
		}

		// decrement the space left variable
		the_queue->available--;
...
void * worker_main (void * arg)
{
	...
	while (!params->worker_done) {
		...
		struct image * working_img = images[req.request.img_id];
		uint8_t error;

		if (req.request.img_op == IMG_ROT90CLKW){
			
			// make duplicate to change if overwrite is false
			if (!req.request.overwrite){
				working_img = cloneImage(working_img, &error);
				if (error){
					perror("Clone image failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
				// operate on the image
				working_img = rotate90Clockwise(working_img, &error);
				images[req.request.img_id] = working_img;
				image_count+=1;
			}
			...
			if (error){
					perror("Rotate 90 Clockwise image failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
		}
		else if (req.request.img_op == IMG_BLUR){
			// make duplicate to change if overwrite is false
			if (!req.request.overwrite){
				working_img = cloneImage(working_img, &error);
				if (error){
				...
				// operate on the image
				working_img = blurImage(working_img, &error);
				images[req.request.img_id] = working_img;
				image_count+=1;
			}
			...
			// check that is was successful
			if (error){
					perror("blurImage failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
		}
		else if (req.request.img_op == IMG_SHARPEN){
			// make duplicate to change if overwrite is false
			if (!req.request.overwrite){
				working_img = cloneImage(working_img, &error);
				if (error){
				...
				// operate on the image
				working_img = sharpenImage(working_img, &error);
				images[req.request.img_id] = working_img;
				image_count+=1;
			}
			...
			// check that is was successful
			if (error){
					perror("sharpenImage failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
		}
		else if (req.request.img_op == IMG_VERTEDGES){
			// make duplicate to change if overwrite is false
			if (!req.request.overwrite){
				working_img = cloneImage(working_img, &error);
				if (error){
				...
				// operate on the image
				working_img = detectVerticalEdges(working_img, &error);
				images[req.request.img_id] = working_img;
				image_count+=1;
			}
			...
			// check that is was successful
			if (error){
					perror("detectVerticalEdges failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
			
		}
		else if (req.request.img_op == IMG_HORIZEDGES){
			// make duplicate to change if overwrite is false
			if (!req.request.overwrite){
				working_img = cloneImage(working_img, &error);
				if (error){
				...
				// operate on the image
				working_img = detectHorizontalEdges(working_img, &error);
				images[req.request.img_id] = working_img;
				image_count+=1;
			}
			...
			// check that is was successful
			if (error){
					perror("detectHorizontalEdges failed");
					ERROR_INFO();
					return (void *)EXIT_FAILURE;
				}
			
		}
		else if (req.request.img_op == IMG_RETRIEVE){
			working_img = images[req.request.img_id];
			uint8_t success = saveBMP(`eval1-${req.request.img_id}.png`, working_img);
			printf("%d", success);
		}
		else{
		...
		if (req.request.img_op == IMG_RETRIEVE) {
			sendImage(working_img, params->conn_socket);
		} 

		// print to client
sync_printf("T%d R%ld:%lf,%d,%d,%ld,%ld,%lf,%lf,%lf\n", params->worker_id, req.request.req_id,
	...
	do {
		...
		if (in_bytes > 0) {
			...
			if (req->request.img_op == IMG_REGISTER){
				// receive the image
				clock_gettime(CLOCK_MONOTONIC, &req->start_timestamp);
				struct image * img_ptr =  recvImage(conn_socket);
				images[image_count] = img_ptr;
				image_count+=1;
				...
				// print to client
				sync_printf("T0 R%ld:%lf,%d,%d,%ld,%d,%lf,%lf,%lf\n", req->request.req_id,
					TSPEC_TO_DOUBLE(req->request.req_timestamp),
					req->request.img_op,
					req->request.overwrite,
					req->request.img_id,
