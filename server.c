...
static void handle_connection(int conn_socket)
{
	...
	struct request req;
	int status;
	status = recv(conn_socket, &req, sizeof(req), MSG_WAITALL);
	...
	while (status > 0){
		...
		res.id = req.id;
		res.field = 0;
		res.acknowledgement = 0;
		success = send(conn_socket, &res, sizeof(res), 0);	
		if (success == -1){
			ERROR_INFO();
			printf("Unsuccessful response sent");
		}

		// get completion timestamp
		clock_gettime(CLOCK_MONOTONIC, &completion_timestamp);		
		
		printf("R%lu:%.9f,%.9f,%.9f,%.9f\n",req.id, TSPEC_TO_DOUBLE(req.begin_timestamp),TSPEC_TO_DOUBLE(req.length),TSPEC_TO_DOUBLE(receipt_timestamp),TSPEC_TO_DOUBLE(completion_timestamp));
