...
int add_to_queue(struct request_meta to_add, struct queue * the_queue)
{
	...
	/* OPTION 2: Find where to place the request in the
	   ...
	   * position to the right. */
			...
			// find the spot where the request is longer (greater length) than the new request 
			// int i = the_queue->rd_pos;
			// struct timespec new_req_len = to_add.request.req_length;
			// while( timespec_cmp(the_queue->requests[i].req_length, new_req_len) != 1){
			// 	i = (i + 1)  % the_queue->max_size;
			// }
			// end = (wr_pos + 1) % the_queue->max_size;
			// while()
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	the_queue->policy = conn_params.policy;
...
int main (int argc, char ** argv) {
	...
	while((opt = getopt(argc, argv, "q:w:p:")) != -1) {
		...
		case 'w':
			...
			break;
		case 'p':
			policy_choice = strdup(optarg);
			if (strcmp(policy_choice, "FIFO")==0){
				conn_params.policy = QUEUE_FIFO;
			}
			else if (strcmp(policy_choice, "SJN")==0){
